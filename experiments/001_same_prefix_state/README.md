...
Status: P0, R0, and R1 capability checks completed. M engineering validation is running; state-effect hypotheses remain unmeasured.
...
The [R0 report](results/R0_REPORT.md) reproduces the static audit and finds 76 Easy and 61 Hard potential M roots, while causal F eligibility remains zero. The [R1 capability report](results/R1_REPORT.md) records nonzero official-task performance and passing inference parity; R2 decomposes route difficulty with a 256-generation ceiling. R3 tests same-text candidates independently of route competence. Full generated blocks (F) and first mixed blocks with fixed prompt latents (M) are separate cohorts. R4 measures and independently confirms future-distribution effects; R5 conditionally tests reward gain and training-free compute value.

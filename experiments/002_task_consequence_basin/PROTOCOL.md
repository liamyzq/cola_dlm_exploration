...
The auxiliary confidence model uses fixed L2 logistic regression with C=1, maximum 2,000 iterations, standardized numeric features, one-hot token/field/domain/template/order features, and four world-grouped folds. Add task role to the same baseline containing original/replacement token, field, domain, template, source/answer order, position, clean gap and entropy. Freeze these settings before test; report held-out Bernoulli log-loss/Brier increments and clean-gap overlap. This is predictive adjustment rather than causal mediation.

## Auxiliary geometry model freeze

Before accessing H2 test outcomes, fix the post-recovery geometry model to the same development-established four-world-grouped-fold L2 logistic recipe (C=1, maximum 2,000 iterations). The baseline uses domain, field, original token, noise fraction, residual RMS, clean original-alternative gap, entropy and position. Add the target-specific ray upper bound and its missing indicator, fixed-search success at RMS 0.4/0.8/1.6, and the clean original-alternative margin-gradient Frobenius norm over all answer latents. No hyperparameter search is performed. Analyze all four recovery stages together on the 128-world boundary cohort; report total damage and valid-wrong outcomes separately, probability-score increments with world intervals, pooled and per-fold AUROC, and calibration bins. A single-class training fold is unestimable; a single-class evaluation fold has undefined AUROC. This is an auxiliary target-specific proxy, not a distance to every damaging boundary.
...
## Parser-v2 correction after blinded review

The frozen100-item assistant audit agrees with the original parser on96 items. Five fields in four items expose incomplete entity phrases: dangling `and`, a dangling possessive, and standalone `of` were incorrectly labeled valid-wrong. Correct the shared entity normalization to reject standalone `of`, coordinating `and`/`or`, and terminal apostrophe-s. Preserve all raw outputs and original scores; rescore every retained parser-scored record and clean anchor across every development/test/control run in a separate versioned view. Exact-token search and clean eligibility do not use this value parser and remain unchanged. Keep the original100 audit items rather than resampling after correction.

All80 original parser cases,10 incomplete/complete-name regression cases, and all200 blind-review field labels agree under the corrected scorer. Other alphabetic entity strings remain literal parsed labels, not guarantees of plausible person names. Whole-fact judgments in the assistant audit are therefore reported separately. The correction is post-hoc evaluator repair under the declared audit policy; report whether total damage or either primary effect changes after uniform rescoring.
